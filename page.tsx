...
export default function Home() {
  ...
  const fetchStudyPlan = async () => {
    ...
    try {
      ...
      setPlan(result); // Update the plan state with the parsed data
    } catch (error) {
      console.log(error);
    }
    setLoading(false);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-24">
      ...
      <h2 className="text-xs text-gray-600 mb-5">the AI study planner</h2>
      <div className="w-full md:w-1/2 flex flex-col gap-3">
        ...
        {plan.length > 0 && <Table plan={plan} />} {/* Render the Table component if plan has data */}
      </div>
    </main>
  );
}